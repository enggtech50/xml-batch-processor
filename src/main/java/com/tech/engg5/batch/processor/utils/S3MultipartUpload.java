...
import com.tech.engg5.batch.processor.exception.S3MultipartUploadException;
...
public class S3MultipartUpload {
  ...
  public S3MultipartUpload(String bucketName, String key, AmazonS3 s3Client, Config config) {
    var threadPoolExecutor = new ThreadPoolExecutor(
      config.threadCount, config.threadCount, 0L, TimeUnit.MILLISECONDS,
  ...
  public void uploadPart(byte[] bytes, int partNumber) { uploadPart(new ByteArrayInputStream(bytes), partNumber); }
  ...
  private void submitUploadPart(ByteArrayInputStream inputStream, int partNumber, boolean finalPart) {
    submitTask(() -> {
      ...
      try {
        ...
        LOG.info("Submitted partNumber - {}, uploadId - {}. Result - {}", partNumber, uploadId, uploadPartResult.getETag());
